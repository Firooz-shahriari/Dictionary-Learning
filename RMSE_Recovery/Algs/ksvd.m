 function [Gamma,Dout,err,gerr,ratio] = ksvd(params,varargin)
...
%      problem, while the approximate update performs alternate-optimization
...
data = params.data;
Dout = zeros(size(data,1),params.dictsize, params.iternum);
...
% err(iter)   = 20*log10(ndata/norm(data-D*Gamma,'fro'));
  err(iter)  = norm(data-D*Gamma,'fro')/sqrt(numel(data));
  end
...
%    if params.up
%       D1=(data*Gamma')/(Gamma*Gamma'+1e-2*eye(size(D,2)));
%       D1=D1*diag(sqrt(1./sum(D1.*D1)));
%       D=0.1*D1+0.7*D;
