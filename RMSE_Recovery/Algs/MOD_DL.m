function [D,Gamma,err,gerr,ratio] = MOD_DL(params,varargin)
%  Method of Optimal Directions (MOD) algorithm for dictionary learning.
%  [D,GAMMA] = MOD_DL(PARAMS) runs the MOD_DL dictionary training algorithm on
%  the specified set of signals, returning the trained dictionary D and the
%  signal representation matrix GAMMA.
%
%  MOD_DL has two modes of operation: sparsity-based and error-based. For
...
%
%  [D,GAMMA,ERR] = MOD_DL(PARAMS) also returns the target function values
...
%
%  [D,GAMMA,ERR,GERR] = MOD_DL(PARAMS) computes the target function values on
...
%
%  [...] = MOD_DL(...,VERBOSE) where VERBOSE is a character string, specifies
...
%  example, MOD_DL(PARAMS,'tr') prints the iteration number, number of
...
%
%  [...] = MOD_DL(...,MSGDELTA) specifies additional messages to be printed
...
%  causes RLMC_DL to run in silent mode, ignoring the value of MSGDELTA.
%
%
...
%      Specifies the number of MOD_DL iterations to perform. If not
...
for iter = 1:iternum
  ...
  s1=(data*Gamma');
  s2=(Gamma*Gamma')+1e-4*eye(dictsize);
  D=s1/s2;
  D = normcols(D);
