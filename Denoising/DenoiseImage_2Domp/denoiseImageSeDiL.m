...
function [IOut,output] = denoiseImageSeDiL(Image,sigma,iteraion,varargin)
...
ms         = mean(blkMatrix);
blkMatrix  = bsxfun(@minus,blkMatrix,ms);
blkMatrix  = bsxfun(@times,blkMatrix,1./(sqrt(sum(blkMatrix.^2))));
...
DCT     = zeros(bb,2*bb);
for kk  = 0:1:2*bb-1
    V   = cos((0:1:size(inp,1)-1)'*kk*pi/bb);
...
para.max_iter = iteraion;
para.mu       = 1e2;                                               % Multiplier in log(1+mu*x^2)
para.lambda   = 0.0135;%0.00039; %5e-2;  %1e9;%1e-3;                        % Lagrange multiplier
para.kappa    = 0.0129;%0.00039; %1e-1;    %1e4                            % Weighting for Distinctive Terms
para.q        = [0,1];
% para.D{1}     = DCT;
% para.D{2}     = DCT;
para.D{1}     = normc(randn(bb,2*bb));
para.D{2}     = normc(randn(bb,2*bb));
para.verbose  = 10;

Learn_para  = learn_separable_dictionary(inp, para);
